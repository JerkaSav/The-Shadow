...
export const OnboardingStart: React.FC<Props> = ({ update }) => {
  const history = useHistory();
  const [imgShown, setImgShown] = useState(inventory);
  const [textDisplayed, setTextDisplayed] = useState(
    `The trusty backpack that every adventures needs! Click on me to review its contents`
  );
  ...
  function ChangeImg() {
    switch (imgShown) {
      case inventory:
        setImgShown(logBook);
        setTextDisplayed(
          "So you dont have to remember everything. Click on this when you feel lost"
        );
        update("Logbook");
        break;
      case logBook:
        ...
        setTextDisplayed(
          "Time to change those old rags? Click here to review what your hand is holding"
        );
        update("Equipment");
        break;
      default:
